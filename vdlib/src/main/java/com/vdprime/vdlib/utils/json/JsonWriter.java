...
{
    final Writer writer;
    private final List<JsonObject> stack = new ArrayList<JsonObject>();
    ...
    static public enum OutputType
    {
        ...
        json, /** Like JSON, but names are only double quoted if necessary. */
    javascript, /**
     * Like JSON, but:
     * <ul>
    ...
    minimal;
			...
			*/
                ...
                !string.contains("/*"))
            {
                ...
                if (length > 0 &&
                    ...
                    minimalValuePattern.matcher(buffer)
                                       .matches()) { return buffer.toString(); }
            }
            return '"' +
                   buffer.replace("\"", "\\\"")
                         .toString() +
        ...
        public String quoteName(String value)
        {
			...
			buffer.replace('\\', "\\\\").replace('\r', "\\r").replace('\n', "\\n").replace('\t', "\\t");
			*/
            String buffer = value.replaceAll("\\\\", "\\\\\\\\");
            switch (this)
            {
                case minimal:
                    if (!value.contains("//") &&
                        !value.contains("/*") &&
                        minimalNamePattern.matcher(buffer)
                                          .matches()) { return buffer.toString(); }
                case javascript:
                    if (javascriptPattern.matcher(buffer)
                                         .matches()) { return buffer.toString(); }
            }
            return '"' +
                   buffer.replace("\"", "\\\"")
